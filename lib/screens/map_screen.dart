...
/// Já pedimos a localização alguma vez nesta instalação. O booleano sozinho não
/// dizia QUAL instalação: o Auto Backup devolve as prefs numa reinstalação com
/// a permissão zerada, e o boot nunca mais perguntava (medido em 23/09/2026 no
/// aparelho do Pedro: `presence_write` negado em toda sessão pós-restore). Por
/// isso a marca agora guarda a hora da instalação em que foi feita.
const _kLocationAsked = 'location_asked';
const _kLocationAskedInstall = 'location_asked_install';

/// Puro/testável: a marca de "já pedimos" vale pra ESTA instalação?
///
...
class _MapScreenState extends State<MapScreen> with WidgetsBindingObserver {
  ...
  static Future<int?> _installTimeMs() async {
    try {
  ...
  Future<void> _markLocationAsked() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_kLocationAsked, true);
    final inst = await _installTimeMs();
    if (inst != null) await prefs.setInt(_kLocationAskedInstall, inst);
  }

  Future<bool> _locationAlreadyAsked() async {
    final prefs = await SharedPreferences.getInstance();
    final inst = await _installTimeMs();
    final marcadoEm = prefs.getInt(_kLocationAskedInstall);
    final legado = prefs.getBool(_kLocationAsked) ?? false;
    ...
    if (marcadoEm == null && legado && inst != null) {
      await prefs.setInt(_kLocationAskedInstall, inst);
    }
    return locationAskedThisInstall(
        marcadoEm: marcadoEm, instalacao: inst, legado: legado);
  }
